...
abstract class BasePagesPeer {
	...
	const CLASS_DEFAULT = 'lib.model.Pages';
	...
	/** the column name for the IS_301REDIRECT field */
	const IS_301REDIRECT = 'pages.IS_301REDIRECT';
	...
	/** the column name for the BREADCRUMB field */
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(PagesPeer::H1);
		...
		$criteria->addSelectColumn(PagesPeer::CREATED_AT);
		$criteria->addSelectColumn(PagesPeer::UPDATED_AT);
	}
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BasePagesPeer', $criteria, $con);
		}
	 ...
	 */
	public static function addInstanceToPool(Pages $obj, $key = null)
	{
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		$cls = PagesPeer::getOMClass(false);
	...
	{
		return $withPrefix ? PagesPeer::CLASS_DEFAULT : PagesPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a Pages or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BasePagesPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BasePagesPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BasePagesPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BasePagesPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Pages or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BasePagesPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BasePagesPeer', $values, $con))
      {
        return $sf_hook_retval;
		...
		if ($values instanceof Criteria) {
			...
			$selectCriteria->add(PagesPeer::ID, $criteria->remove(PagesPeer::ID), $comparison);
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BasePagesPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BasePagesPeer', $values, $con, $ret);
    }
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(PagesPeer::TABLE_NAME, $con);
	...
	/**
	 * Method perform a DELETE on the database, given a Pages or Criteria object OR a primary key value.
	 *
	 ...
	 */
	public static function doValidate(Pages $obj, $cols = null)
	{
		$columns = array();
	...
	{
	  return array(array('old_link'), array('slug'));
	}
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BasePagesPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
