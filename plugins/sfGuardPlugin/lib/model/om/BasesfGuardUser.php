...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	 ...
	 * related FilledForms from storage. If this sfGuardUser is new, it will return
		...
		if ($this->collFilledForms === null) {
			if ($this->isNew()) {
			   $this->collFilledForms = array();
				...
				$criteria->add(FilledFormPeer::OPERATOR_ID, $this->id);
	 ...
	 */
	public function getFilledFormsJoinServiceForm($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collFilledForms === null) {
			if ($this->isNew()) {
				...
				$this->collFilledForms = FilledFormPeer::doSelectJoinServiceForm($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
